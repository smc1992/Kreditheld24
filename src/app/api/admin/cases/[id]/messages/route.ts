    ...
    await db.update(crmCaseMessages)
      ...
      .where(
        and(
          ...
          eq(crmCaseMessages.senderType, 'customer'),
...
export async function POST(
  ...
  try {
    ...
    // Send email notification to customer if they have verified email
    if (caseData.customer?.email && caseData.customer?.emailVerified) {
    ...
    return NextResponse.json({ 
      ...
      notificationSent: caseData.customer?.emailVerified || false
    });
  } catch (error) {
